...
export default function JobCard({ job, refreshJobs }: { job: any; refreshJobs: () => void }) {
  ...
  const subtotal = services.reduce((s: number, it: any) => s + (Number(it.totalPrice) || 0), 0);
  const total = typeof job.totalOverride === "number" && job.totalOverride !== null ? job.totalOverride : (job.invoice?.total ?? subtotal);
        ...
        <CardContent className="space-y-4 p-6">
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-xl flex items-center gap-2">
                  {job.carNumber}
                  {isHovered && (
                    <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-xs bg-gradient-to-r from-pink-500 to-purple-500 text-white px-2 py-1 rounded-full">
            ...
            <div className="text-right">
              <motion.span className={`px-3 py-1.5 text-xs rounded-full font-medium`}>
                {statusText}
              </motion.span>
              <div className="mt-2 text-sm">Total: <span className="font-semibold">{Number(total || 0).toFixed(2)}</span></div>
            </div>
          </div>
            ...
            {assignedTo && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="truncate font-medium">{job.assignedTo?.email || assignedTo}</p>
                </div>
              </div>
            )}
          </div>
          ...
          {services.length > 0 && (
            <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg text-sm border border-gray-200 dark:border-gray-700">
              <p className="font-medium flex items-center gap-2"><Wrench className="w-4 h-4" /> Services</p>
              <p className="text-muted-foreground mt-1">
                {services.slice(0, 2).map((s: any) => s.name).join(", ")}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {isTeam && job.status === "pending" && !assignedTo && (
              <button onClick={handleClaim} className="px-4 py-2 bg-blue-600 text-white rounded">Claim</button>
            )}

            {isTeam && job.status === "in_progress" && assignedTo === userId && (
              <button onClick={handleComplete} className="px-4 py-2 bg-green-600 text-white rounded">Complete</button>
            )}

            <button onClick={() => window.open(`/api/pdf/${job._id}`, "_blank")} className="px-4 py-2 bg-gray-700 text-white rounded">Download PDF</button>

            {isAdmin && job.status === "completed" && (
              <>
                <button onClick={handleAccept} className="px-4 py-2 bg-green-700 text-white rounded">Accept</button>
            ...
            {isAdmin && (
              <>
                <button onClick={handleEdit} className="px-4 py-2 bg-indigo-600 text-white rounded">Edit</button>
